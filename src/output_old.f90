...
MODULE output
  ...
  SUBROUTINE plot(BOsigma,Rcl,Vcl,time)
    ...
    REAL(KIND=DP)                  :: Rel_co(2), Mag_co(2)
    ...
    DO i=1,nstates
       DO j=i+1,nstates
          ...
          DO itraj=1,ntraj
             ...
             BO_coh_ind(index_ij) = BO_coh_ind(index_ij) + &
    ...
    BO_coh_ind = BO_coh_ind/dble(ntraj)
    ...
    DO itraj=1,ntraj
      ...
      Rel_co(1) = Rel_co(1) + &
        DSQRT((real(BOsigma(itraj,2,2),KIND=dp))* &
         (real(BOsigma(itraj,6,6),KIND=dp)))
      Rel_co(2) = Rel_co(2) + &
     DSQRT((real(BOsigma(itraj,2,2),KIND=dp))* &
         (real(BOsigma(itraj,14,14),KIND=dp)))
      Comp_co(1) = Comp_co(1) + &
        BOsigma(itraj,2,6)
      Comp_co(2) = Comp_co(2) + &
        BOsigma(itraj,2,14)
  ...
  SUBROUTINE initialize_output
    ...
    WRITE(88,*) "#Time, Coherences (S1-S5, S1-S13, S5-S13) SUMMAG MAGSUM"
    ...
    OPEN(94,FILE=TRIM(output_folder)//"/BO_coherence_ind.dat",STATUS="replace", &
      FORM="formatted",ACTION="write",IOSTAT=ios)
    IF(ios/=0) PRINT*,'error opening BO_coherence_ind.dat'
    WRITE(94,*) "#Time, Coherence indicator"
